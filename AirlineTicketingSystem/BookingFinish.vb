Public Class BookingFinish
    ...
    Private Sub BookingFinish_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        body.AppendLine("Total price: <h3>" & Booking.TotalPrice.ToString("RM 0.00") & "</h3>")
        body.AppendLine("Leaves from <b> " & DB.GetFlightSource(Booking.FlightID).City.Name & "<b/> on <b>" & Booking.Flight.DepartureTime.ToString("dd/MM/yyyy hh:mm ss") & "</b><br>")
        body.AppendLine("Arrives at <b> " & DB.GetFlightDestination(Booking.FlightID).City.Name & "<b/> on <b>" & Booking.Flight.ArrivalTime.ToString("dd/MM/yyyy hh:mm ss") & "</b><br>")
        body.AppendLine("<hr>")
